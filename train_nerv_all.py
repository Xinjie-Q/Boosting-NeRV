...
def main():
    ...
    parser.add_argument('--cabac', action='store_true', default=False, help='do interpolation')
    ...
    parser.add_argument('--dump_values', action='store_true', default=False, help='dump the prediction images')
...
def evaluate(model, full_dataloader, local_rank, args, 
    ...
    for model_ind, cur_model in enumerate(model_list):
        ...
        for i, sample in enumerate(full_dataloader):
            ...
            if i > 10 and args.debug:
                break
            if 'pre_img' in sample.keys() and 'post_img' in sample.keys():
                pre_img, post_img = data_to_gpu(sample['pre_img'], device), data_to_gpu(sample['post_img'], device)
            img_data, img_gt, inpaint_mask = args.transform_func(img_data, img_idx)
            if 'pe' not in args.embed or "HNeRV_Boost" in args.model:
                cur_input = img_data
            else:
            ...
            if args.interpolation and args.embed_inter and img_idx.item() in args.val_ind_list:
                img_out, embed_list, dec_time = cur_model(cur_input, dequant_vid_embed[i] if model_ind and "HNeRV" in args.model else None, 
                    pre_img=pre_img, post_img=post_img, norm_idx=norm_idx)
            else:
                img_out, embed_list, dec_time = cur_model(cur_input, dequant_vid_embed[i] if model_ind and "HNeRV" in args.model else None, 
                    norm_idx=norm_idx)
            if model_ind == 0:
            ...
            if args.eval_fps:
                time_list.pop()
                for _ in range(100):
                    _, _, dec_time = cur_model(cur_input, embed_list[0], norm_idx=norm_idx)
                    time_list.append(dec_time)

            # compute psnr and ms-ssim
            pred_psnr, pred_ssim = psnr_fn_batch([img_out], img_gt), msssim_fn_batch([img_out], img_gt)
            #pred_psnr, pred_ssim = psnr_fn_single(img_out, img_data), msssim_fn_single(img_out, img_data)
            #print("pred_psnr:{}, pred_ssim:{}".format(pred_psnr, pred_ssim))
            ...
            if dump_vis:
                for batch_ind, cur_img_idx in enumerate(img_idx):
                    ...
                    concat_img = img_out[batch_ind] #torch.cat(dump_img_list, dim=2)    #img_out[batch_ind], 
        ...
        if model_ind == 0:
            if "HNeRV" in args.model:  
                ...
                if args.dump_values:
                    fea_embed = vid_embed.cpu().numpy().flatten()
                    fig = plt.figure()
                    sns.set_style("darkgrid")
                    sns.distplot(weight, color='b', kde=False, norm_hist=False)
                    plt.savefig(f'{visual_dir_values}/embed.png', dpi=200, bbox_inches='tight', pad_inches=0.05)
                ...
                quant_embed, dequant_emved = torch.zeros(1), torch.zeros(1)
                dequant_vid_embed = torch.zeros(1)
